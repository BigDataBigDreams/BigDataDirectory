...
(function() {
  var SELECT_MODEL, USE_MODEL, USE_STARS, addNewAppliance, getStarInput, modelInput;
  ...
  window.getNewRow = function() {
    ...
    newRow = $('<tr>').append($('<td>').html('<select id="applianceType">\
			...
			</select>')).append($('<td>').html(modelInput())).append($('<td>').append($('<a>').html(USE_STARS).attr('href', "javascript:void(0);").addClass('use-stars-button').click(function() {
      var jthis;
      jthis = $(this);
      if (jthis.html() === USE_STARS) {
        ...
        return jthis.html(USE_STARS);
      }
    })), newRow.find('.input').html(getStarInput())).append($('<td>'));
    return newRow;
  };
